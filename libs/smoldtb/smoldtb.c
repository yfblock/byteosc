 ...
 *
 * The public API only supports a single (global) active parser, but internally
 * everything is stored within an instance of the 'dtb_state' struct. This
 * should make it easy to support multiple parser instances in the future if
 * needed. Or if you're here to hack that support in, it should hopefully
...
/* Similar to the above function, but returns success if it reaches the end of
 * the string OR it reaches the length.
 */
...
/* Allocator design:
 * The parser allocates a single large buffer from the host kernel, or uses the
 * compile time statically allocated buffer. It then internally breaks that
 * into 3 other buffers:
 * - the first is `node_buff` which is used as a bump allocator for node
 * structs. This function
 *   (`alloc_node()`) uses this buffer to return new pointers.
 * - the second is `prop_buffer` which is similar, but for property structs
...
static void free_buffers() {
    ...
    if(state.ops.free == NULL) {
        if(state.ops.on_error)
            state.ops.on_error(
                "ops.free() is NULL while trying to free buffers.");
        return;
    }
...
static void alloc_buffers() {
    ...
    if(total_size >= SMOLDTB_STATIC_BUFFER_SIZE) {
        if(state.ops.on_error)
            state.ops.on_error(
                "Too much data for statically allocated buffer.");
        return;
    }
    uint8_t *buffer = big_buff;
#else
    uint8_t *buffer = (uint8_t *)state.ops.malloc(total_size);
#endif
...
void dtb_init(uintptr_t start, dtb_ops ops) {
    ...
    for(size_t i = 0; i < state.cell_count; i++) {
        ...
        dtb_node_t *sub_root = parse_node(&i, 2, 1);
        if(sub_root == NULL)
            continue;
...
dtb_node_t *dtb_find_compatible(dtb_node_t *start, const char *str) {
...
dtb_node_t *dtb_find_phandle(unsigned handle) {
    ...
    return NULL; // TODO: would it be nicer to just search the tree in this
                 // case?
}
...
dtb_node_t *dtb_find_child(dtb_node_t *start, const char *name) {
    if(start == NULL)
        return NULL;
...
void dtb_stat_node(dtb_node_t *node, dtb_node_stat *stat) {
    ...
    if(node->parent) {
        dtb_node_t *prime = node->parent->child;
        while(prime != NULL) {
            prime = prime->sibling;
            stat->sibling_count++;
        }
...
size_t dtb_read_prop_cell(const uint32_t *cells, size_t count) {
    return extract_cells(cells, count);
}
...
size_t dtb_read_prop_pairs(dtb_prop_t *prop, dtb_pair layout, dtb_pair *vals) {
    ...
    const struct fdt_property *fdtprop =
        (const struct fdt_property *)(prop->first_cell - 2);
    const size_t count =
        be32(fdtprop->length) / ((layout.a + layout.b) * FDT_CELL_SIZE);
    if(vals == NULL)
        return count;
